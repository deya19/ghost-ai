...
- Bug fix (Prisma lazy proxy specs route): Updated `lib/prisma.ts` lazy Prisma proxy with a `has` trap so route checks like `"projectSpec" in prisma` resolve against the real Prisma client instead of the empty proxy target. This prevents the specs list API from falsely returning 500 after switching Prisma to lazy initialization for Trigger.dev task indexing compatibility
